...
def shuffle_data_labels(data, labels, rng):
    indices = np.arange(len(data))
    rng.shuffle(indices)
    return data[indices].copy(), labels[indices].copy()
...
def make_diag_mask(size, radius):
    ...
    abs_idx_diff = (ids.unsqueeze(0) - ids.unsqueeze(1)).abs()
...
def copy_data_to_device(data, device):
    if torch.is_tensor(data):
        ...
        return Variable(torch.from_numpy(data).to(device))
    elif isinstance(data, (list, tuple)):
        return Variable([copy_data_to_device(elem, device)
                         for elem in data], dtype=object)
